...
class Dashboard(object):
    ...
    def create_row(self, gpio):
        ...
        html += "<input type='hidden' name='gpio' value='%s'>" % gpio
        if self.gpio_pins[gpio]['state'] == "0":
            state_col = 'red'
        else:
            state_col = 'green'
        html += "<td style='background-color:%s'>" % state_col
        html += "<input name='send' type='submit' value='flip'></td>"
        html += """
                <td>"""
        ...
        for dow in ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']:
            ...
            if 'dow' in self.gpio_pins[gpio].keys() and \
               re.match(".*%s.*" % dow, self.gpio_pins[gpio]['dow']):
                checked = " checked"
            html += "<input type='checkbox' name='dow_%s' value='%s'%s>%s" % (dow, dow, checked, dow)
        html += "</td>"
        html += """
    ...
    def create_tab(self, get_back=False):
        """ Creates table to show the different gpiopins
        get_back: If true return created html instead of storing
        it within self.html
        """
        html = ""
        for gpio in self.gpio_pins.keys():
            html += self.create_row(gpio)
        if get_back:
            return html
        else:
            self.html += html
